...
export default function MessageContent({ message }: MessageContentProps) {
    ...
    return (
        <div className={cn("flex items-center gap-2", `${message.role === "user" && 'ml-auto flex-row-reverse'}`)}>
            ...
            <p className={`text-sm ${message.role === "user" && 'text-right'} sm:text-start sm:text-md max-w-[170px] sm:max-w-[300px] p-4 border-[1.5px] border-slate-300 rounded-xl ${message.role === "user" ? "rounded-tr-none" : "rounded-tl-none"}`}>
                {message.content}
            </p>
        </div>
    );
